...
def check_set_params():
    ...
    wps_nml['geogrid']['geog_data_path'] = '/WPS_GEOG'
    ...
    # if date_diff.seconds/60 < params.file['time_control']['history_file']['history_begin']:
    ...
    with open(params.data_path.joinpath('namelist.wps'), 'w') as nml_file:
       wps_nml.write(nml_file)
    
    with open(params.data_path.joinpath('namelist.input'), 'w') as nml_file:
       wrf_nml.write(nml_file)

    # return start_date, end_date, int(interval_secs/60/60)
